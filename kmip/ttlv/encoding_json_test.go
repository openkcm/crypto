...
func (s *JsonDecodingSuite) TestDecodeBigInteger() {
	s.Run("positive", func() {
		...
		s.NoError(err)
		s.Equal("1234567890000000000000000000", n.String())
	})
	s.Run("zero", func() {
...
func (s *JsonDecodingSuite) TestDecodeDateTime() {
	...
	s.NoError(err)
	s.Equal(time.Date(2008, time.March, 14, 11, 56, 40, 0, time.UTC), n.UTC())
}
