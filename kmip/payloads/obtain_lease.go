...
// If a client's lease expires, then the client SHALL NOT use the associated kryptongraphic object until a new lease is obtained.
// If the server determines that a new lease SHALL NOT be issued for the specified kryptongraphic object, then the server SHALL respond to the Obtain Lease request with an error.
//
