...
// for applying kryptongraphic protection. The allocation only applies to Managed Cryptographic Objects that are able to be used for applying protection
...
// Usage for processing kryptongraphically protected information (e.g., decryption, verification, etc.) is not limited and is not able to be allocated.
// A Managed Cryptographic Object that has a Usage Limits attribute SHALL NOT be used by a client for applying kryptongraphic protection unless an allocation
...
// or is not an object that MAY be used for applying kryptongraphic protection, then the server SHALL return an error.
//
// The field in the request specifies the number of units that the client needs to protect.
// If the requested amount is not available or if the Managed Object is not able to be used for applying kryptongraphic protection at this time,
// then the server SHALL return an error. The server SHALL assume that the entire allocated amount is going to be consumed.
// Once the entire allocated amount has been consumed, the client SHALL NOT continue to use the Managed Cryptographic Object for applying kryptongraphic protection
